...
class ImageEditor(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.btn_green.clicked.connect(self.green)
        self.ui.btn_red.clicked.connect(self.red)
        self.ui.btn_blue.clicked.connect(self.blue)
    ...
    def red(self):
        if self.original:
            red = self.original.split()
            zeroed_band = red[1].point(lambda _: 0)
            self.original = Image.merge("RGB", (red[1], zeroed_band, zeroed_band))
            self.save_image()
            self.show_image()

    def blue(self):
        if self.original:
            blue = self.original.split()
            zeroed_band = blue[1].point(lambda _: 0)
            self.original = Image.merge("RGB", (zeroed_band, zeroed_band, blue[1]))
            self.save_image()
            self.show_image()
    ...
    def purple_filter(self):
        if self.original:
            ...
            green = green.point(lambda p: p * 0.2)
    ...
    def yellow_filter(self):
        if self.original:
            ...
            blue = blue.point(lambda p: p * 0.2)
            self.original = Image.merge("RGB", (red, green, blue))
            self.save_image()
            self.show_image()
    ...
    def orange_filter(self):
        if self.original:
            ...
            red, green, blue = self.original.split()
            red = red.point(lambda p: p * 1.2) 
            green = green.point(lambda p: p * 1.0)  
            blue = blue.point(lambda p: p * 0.2)  
            self.original = Image.merge("RGB", (red, green, blue))
            self.save_image()
            self.show_image()
